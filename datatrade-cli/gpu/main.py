"""
main.py — Claude Code–style REPL entry point.
...
[bold]Available commands[/bold]
  ...
  [cyan]/cache[/cyan]                       Show all mock-Redis keys + TTL
  [cyan]/cache clear[/cyan]                 Flush mock-Redis
...
class REPL:
    ...
    # ── Lazy singletons (don't load heavy models until first use) ─────────────

    def _get_pipeline(self):
        if self._pipeline is None:
            from modules.pipeline import Pipeline
            with console.status("[cyan]Loading pipeline models…[/cyan]"):
    ...
    def _get_embedder(self):
        if self._embedder is None:
            ...
            with console.status("[cyan]Loading BGE-M3…[/cyan]"):
    ...
    def _get_reranker(self):
        if self._reranker is None:
            ...
            with console.status("[cyan]Loading reranker…[/cyan]"):
    ...
    def _cmd_gpu(self, _):
        ...
        if DEVICE == "cuda":
            console.print(f"[dim]GPU:[/dim] {torch.cuda.get_device_name(0)}")
    ...
    def _cmd_search(self, args):
        ...
        with console.status("[cyan]Embedding query…[/cyan]"):
    ...
    def _cmd_embed(self, args):
        ...
        t = Table("token", "weight", show_header=True, header_style="bold")
    ...
    def _cmd_crawl(self, args):
        ...
        with console.status("[cyan]Embedding + indexing…[/cyan]"):
    ...
    # /index <info|clear temp|clear main|clear all>
    ...
    def run(self):
        ...
        console.print(f"[dim]Device: {DEVICE}  |  type /help for commands[/dim]\n")
